        ...
        try {
            ...
            if (templatePath) {
                try {
                    ...
                    this.logger.warn(`Failed to load or compile email template: ${templatePath}`, templateError instanceof Error
            ...
            this.logger.log(`Email sent successfully to ${mailOptions.to} - MessageId: ${mailResult.messageId}`);
        }
        catch (error) {
            const recipient = mailOptions.to;
            const subject = mailOptions.subject || 'No subject';
            this.logger.warn(`⚠️ Failed to send email to ${recipient} with subject "${subject}"`, error instanceof Error ? error.message : String(error));
        }
    }
};
